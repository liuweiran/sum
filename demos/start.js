...
/*app.use(/\/demos\/(.+\/.+\.html)$/, function (req, res) {     //子表达式可以获取供以后使用---加括号的匹配内容会在req.params中取到
...
//app.use('favicon.ico', express.static('./favicon.ico/', { redirect:false }));
app.use('/demos', express.static('./demos/', { redirect:false }));      //{redirect:false} 防止没有后缀名又没以斜杠结尾的时候自动追加斜杠的问题
